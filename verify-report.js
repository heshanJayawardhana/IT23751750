...
console.log('🚀 Starting Report Generation (Verification Mode)...');
...
const flattenTests = (suites) => {
    // Check if suites is undefined or null
    if (!suites) return;
    ...
    suites.forEach(suite => {
        if (suite.specs) {
            suite.specs.forEach(spec => {
                ...
                const result = test.results[0]; // Take first result usually
...
const pdfPath = path.join(PUBLIC_DIR, 'test_report_verified.pdf');
const excelPath = path.join(PUBLIC_DIR, 'test_report_verified.xlsx');
